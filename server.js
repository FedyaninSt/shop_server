...
// const { connectionPool } = require('./pool/dataPool');
...
const dataCheck = async (dataUser, res) => {
    await connectionPool.query('call clothes.UserEntry();', (err, data) => {
        if (err) {
            res.status(500);
        } else {
            let dataBD = null;
...
app.listen(5000, () => {
    console.log('server is running uraaaaa');
})
